...
## Проверка доступов согласно поставленной задачи:
    ...
    2) Обеспечить взаимодействие IP-префиксов клиентов, принадлежащих разным тенантам (VRF) через HA пару МСЭ согласно матрице доступа.
     ...
     Дополнительная проверка взаимодействия клиентов различных тенантов 
...
При этом в любой момент времени монопольным источником маршрута является активная нода МСЭ, импорт префикса префикса 0.0.0.0/0 [Default Route] происходит на основе тригера реализованного через трекинг:  
